...
final validEngAndHeb = RegExp(
    r'^[a-z A-Z \u0590-\u05FF\u200f\u200e ]+$'); // checks for hebrew or english
...
Future<bool> leaveProgress(context, bool removeAll,
    {bool goHomePage = false}) async {
            ...
            actions: <Widget>[
              ...
              TextButton(
                ...
                onPressed: () {
                  ...
                  if (goHomePage) {
                    // only if in favorites and want to go back to home page
...
Map<String, dynamic> makeDict(String category) {
  ...
  Map<String, dynamic> dataDict = {
    ...
    "unit": myProfile.appartment,
    "category": category
  };
  if (visitors.length == 1) {
    dataDict.addAll({
      "guest_id": visitors[0].id,
      "guest_name": visitors[0].name,
      "guest_phone": visitors[0].phone
    });
  }
  if (visitors.length == 2) {
    dataDict.addAll({
      "guest_id2": visitors[1].id,
      "guest_name2": visitors[1].name,
      "guest_phone2": visitors[1].phone
    });
  }
  if (visitors.length == 3) {
    dataDict.addAll({
      "guest_id3": visitors[2].id,
      "guest_name3": visitors[2].name,
      "guest_phone3": visitors[2].phone
    });
  }
  return dataDict;
}
